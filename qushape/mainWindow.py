...
class MainWindow(QtGui.QMainWindow):
    ...
    def readSettings(self):
        ...
        try:
            ...
            size = self.qSettings.value(
                "MainWindow/Size", QtCore.QVariant(QtCore.QSize(600, 500))
            ).toSize()
            self.resize(size)
            position = self.qSettings.value(
                "MainWindow/Position", QtCore.QVariant(QtCore.QPoint(0, 0))
            ).toPoint()
            self.move(position)
    ...
    def writeSettings(self):
        self.qSettings = QtCore.QSettings()
        filename = (
            QtCore.QVariant(QtCore.QString(self.projFileName))
            if self.projFileName is not None
            else QtCore.QVariant()
        )
        self.qSettings.setValue("LastFile", filename)
        recentFiles = (
            QtCore.QVariant(self.recentFiles) if self.recentFiles else QtCore.QVariant()
        )
        self.qSettings.setValue("RecentFiles", recentFiles)
        self.qSettings.setValue("MainWindow/Size", QtCore.QVariant(self.size()))
        self.qSettings.setValue("MainWindow/Position", QtCore.QVariant(self.pos()))
    ...
    def createDockWidgets(self):
        self.dockTool = ToolDock(
            "Tool Inspector", self
        )  # QtGui.QDockWidget("Tool Inspector",self)
        ...
        self.dockScript.setAllowedAreas(
    ...
    def createMenu(self):
        ...
        openProjectAct = self.createAction(
            "&Open Project", self.openProjectDlg, QtGui.QKeySequence.Open, iconOpen
        )
        saveProjectAct = self.createAction(
            "Save Project", self.saveProject, QtGui.QKeySequence.Save, iconSave
        )
        saveProjectAsAct = self.createAction(
            "Save Project As", self.saveProjectAs, QtGui.QKeySequence.SaveAs, iconSaveAs
        )
        ...
        editDrawSatdAction = self.createAction(
            "Draw Saturated Points", self.drawSatd
        )  # , None,None,None, True, "toggled(bool)")
        editDrawSelected = self.createAction(
            "Draw Selected Area", self.drawSelectedArea
        )  # ,None,None,None, True, "toggled(bool)")
        editDrawDefault = self.createAction(
            "Draw Default", self.drawFigure
        )  # ,None,None,None, True, "toggled(bool)")
        ...
        toolSmoothEnhanceAct = self.createAction(
            "Smoothing",
            self.toolActions,
        )
        ...
        toolSignalDecayAct = self.createAction(
            "Signal Decay Correction", self.toolActions
        )
        ...
        toolsMenuActs = (
            toolRoiAct,
            toolSmoothEnhanceAct,
            toolBaselineAct,
            toolSignalDecayAct,
            toolMobilityShiftAct,
            toolSignalAlignmentAct,
            None,
            toolApplyAllAct,
        )
        ...
        seqAlignRefAct = self.createAction(
            "Sequence Alignment by Reference", self.toolActions
        )
        reactAlignRefAct = self.createAction(
            "Reactivity by Reference", self.toolActions
        )
        applyAllSeqAct = self.createAction(
            "Automated Analysis by Reference", self.toolActions
        )
        ...
        toolsManualSignalAct = self.createAction(
            "Manual Signal Alignment", self.toolActions
        )
        toolsVariousToolsAct = self.createAction(
            "Some Useful Functions", self.toolActions
        )

        openShapeFinderAct = self.createAction(
            "Open ShapeFinder File", self.toolActions
        )
        ...
        optionaToolMenuActs = (
            toolsScaleAct,
            toolsSwapAct,
            toolsManualSignalAct,
            toolsVariousToolsAct,
            None,
            openShapeFinderAct,
            openAbifFileAct,
            openFileSeqAct,
        ...
        self.addActions(
            fileToolbar,
            (newProjectAct, openProjectAct, saveProjectAct, saveProjectAsAct),
        )
    ...
    def mainWindowConnect(self):
        self.connect(
            self.mainTopWidget.spinBoxZoom,
            QtCore.SIGNAL("valueChanged(int)"),
            self.setAxesYLim,
        )
        self.connect(
            self.mainTopWidget.spinBoxWidth,
            QtCore.SIGNAL("valueChanged(int)"),
            self.resizeFigure,
        )
        self.connect(
            self.mainTopWidget.spinBoxHeight,
            QtCore.SIGNAL("valueChanged(int)"),
            self.resizeFigure,
        )
        self.connect(
            self.mainTopWidget.checkBoxFitWindow,
            QtCore.SIGNAL("toggled(bool)"),
            self.resizeFigure,
        )
        self.connect(
            self.mainTopWidget.splitComboBox,
    ...
    def onScrollEvent(self, event):
        self.scrollArea.horizontalScrollBar().setValue(
            int(
                ...
                + event.step * self.scrollArea.horizontalScrollBar().pageStep() / 4
            )
        )
    ...
    def onClick(self, event):
        ...
        if (
            self.dlg.name == "Sequence Alignment"
        ):  # or self.lastScript=="Align and Integrate":
            if event.inaxes == self.axesSeq:
                ...
                self.eventKey == QtCore.Qt.Key_Shift
                and event.inaxes == self.chAxes["RX"]
            ):
                self.clickedPeakInd = findClickedInd(
                    x, self.dlg.dProjOut["dPeakRX"]["pos"]
                )
                self.isArrowSelectedRX = True
            elif (
                self.eventKey == QtCore.Qt.Key_Shift
                and event.inaxes == self.chAxes["BG"]
            ):
                self.clickedPeakInd = findClickedInd(
                    x, self.dlg.dProjOut["dPeakBG"]["pos"]
                )
            ...
            if (
                ...
                self.eventKey == QtCore.Qt.Key_Shift
                and event.inaxes == self.chAxes["BGS1"]
            ):
                self.clickedPeakInd = findClickedInd(
                    x, self.dlg.dProjOut["dPeakBG"]["pos"]
                )
            ...
            if self.eventKey == QtCore.Qt.Key_A:
                ...
                if self.clickedXR != None and self.clickedXS != None:
                    self.clickedPeakIndR = findClickedInd(
                        self.clickedXR, self.dlg.linkXR
                    )
                    self.clickedPeakIndS = findClickedInd(
                        self.clickedXS, self.dlg.linkXS
                    )
                    if self.clickedPeakIndR == self.clickedPeakIndS:
                        self.dlg.linkXR = np.insert(
                            self.dlg.linkXR, self.clickedPeakInd, self.clickedXR
                        )
                        self.dlg.linkXS = np.insert(
                            self.dlg.linkXS, self.clickedPeakInd, self.clickedXS
                        )
    ...
    def onMove(self, event):
        ...
        text = (
            "X:"
            + str(np.round(event.xdata, 2))
            + ", Y:"
            + str(np.round(event.ydata, 2))
        )
        ...
        if self.isMouseClick and self.eventKey != QtCore.Qt.Key_Shift:
            for ax, rect in zip(self.fig.get_axes(), self.spanRect):
                ...
                if event.x < self.scrollArea.horizontalScrollBar().value():
                    self.scrollArea.horizontalScrollBar().setValue(
                        int(self.scrollArea.horizontalScrollBar().value() - 5)
                    )
                elif event.x > (
                    self.scrollArea.horizontalScrollBar().value()
                    + self.scrollArea.horizontalScrollBar().pageStep()
                ):
                    self.scrollArea.horizontalScrollBar().setValue(
                        int(self.scrollArea.horizontalScrollBar().value() + 5)
                    )
        ...
        if (
            self.dlg.name == "Sequence Alignment"
        ):  # "Sequence Alignment":#self.dlg.title=="Align and Integrate" or self.lastScript=="Align and Integrate":
            if self.isArrowSelectedRX and event.inaxes == self.chAxes["RX"]:
                self.conFromBGToRX[self.clickedPeakInd].xy2 = (
                    event.xdata,
                    self.dlg.dProjOut["dData"]["RX"][event.xdata],
                )
            ...
            if self.isArrowSelectedBG and event.inaxes == self.chAxes["BG"]:
                self.conFromBGToRX[self.clickedPeakInd].xy1 = (
                    event.xdata,
                    self.dlg.dProjOut["dData"]["BG"][event.xdata],
                )
                self.conFromSeqToBG[self.clickedPeakInd].xy2 = (
                    event.xdata,
                    self.dlg.dProjOut["dData"]["BG"][event.xdata],
                )
                self.chAxes["BG"].draw_artist(self.conFromBGToRX[self.clickedPeakInd])
                self.chAxes["BGS1"].draw_artist(
                    self.conFromSeqToBG[self.clickedPeakInd]
                )

        elif self.dVar["flag"]["isPeakLinkRefModify"]:
            if self.isArrowSelectedRX and event.inaxes == self.chAxes["BG"]:
                self.conRX[self.clickedPeakInd].xy1 = (
                    event.xdata,
                    self.dlg.dProjOut["dData"]["RX"][event.xdata],
                )
                self.chAxes["BG"].draw_artist(self.conRX[self.clickedPeakInd])
            if self.isArrowSelectedBG and event.inaxes == self.chAxes["BGS1"]:
                self.conBG[self.clickedPeakInd].xy1 = (
                    event.xdata,
                    self.dlg.dProjOut["dData"]["BG"][event.xdata],
                )
            ...
            if self.isArrowSelectedR and event.inaxes == self.axesR:
                self.conFromRtoS[self.clickedPeakInd].xy2 = (
                    event.xdata,
                    self.dlg.dataR[event.xdata],
                )
                self.axesR.draw_artist(self.conFromRtoS[self.clickedPeakInd])
            if self.isArrowSelectedS and event.inaxes == self.axesS:
                self.conFromRtoS[self.clickedPeakInd].xy1 = (
                    event.xdata,
                    self.dlg.dataS[event.xdata],
                )
            ...
            if self.isArrowSelectedRX:
                if event.inaxes == self.chAxes["RX"]:
                    argmax = argmax3(
                        ...
                        self.dlg.dProjOut["dData"]["RX"][self.mouseX],
                        self.dlg.dProjOut["dData"]["RX"][self.mouseX + 1],
                    )
                    ...
                    self.dlg.dProjOut["dPeakRX"]["pos"][
                        self.clickedPeakInd
                    ] = self.mouseX
                    self.dlg.dProjOut["dPeakRX"]["amp"][
                        self.clickedPeakInd
                    ] = self.dlg.dProjOut["dData"]["RX"][self.mouseX]
                else:
                    self.conFromBGToRX[self.clickedPeakInd].xy2 = (
                        self.dlg.dProjOut["dPeakRX"]["pos"][self.clickedPeakInd],
                        self.dlg.dProjOut["dPeakRX"]["amp"][self.clickedPeakInd],
                    )
                self.dDrawData = deepcopy(self.dlg.dProjOut)
                self.canvas.draw()
                self.isArrowSelectedRX = False
            if self.isArrowSelectedBG:
                if event.inaxes == self.chAxes["BG"]:
                    argmax = argmax3(
                        ...
                        self.dlg.dProjOut["dData"]["BG"][self.mouseX],
                        self.dlg.dProjOut["dData"]["BG"][self.mouseX + 1],
                    )
                    self.mouseX = self.mouseX - 1 + argmax
                    self.conFromBGToRX[self.clickedPeakInd].xy1 = (
                    ...
                    self.dlg.dProjOut["dPeakBG"]["pos"][
                        self.clickedPeakInd
                    ] = self.mouseX
                    self.dlg.dProjOut["dPeakBG"]["amp"][
                        self.clickedPeakInd
                    ] = self.dlg.dProjOut["dData"]["BG"][self.mouseX]
                else:
                    self.conFromBGToRX[self.clickedPeakInd].xy1 = (
                        self.dlg.dProjOut["dPeakBG"]["pos"][self.clickedPeakInd],
                        self.dlg.dProjOut["dPeakBG"]["amp"][self.clickedPeakInd],
                    )
                self.dDrawData = deepcopy(self.dlg.dProjOut)
                self.canvas.draw()
                self.isArrowSelectedBG = False
    ...
    def newProject0(self):
        ...
        self.connect(
            self.dlg.buttonBox.nextButton, QtCore.SIGNAL("clicked()"), self.newProject1
        )
    ...
    def newProject1(self):
        if self.dlg.isApplied:
            ...
            self.connect(
                self.dlg.buttonBox.nextButton,
                QtCore.SIGNAL("clicked()"),
                self.newProject2,
            )
            self.connect(
                self.dlg.buttonBox.backButton,
                QtCore.SIGNAL("clicked()"),
                self.newProject0,
            )
            self.dockTool.setWidget(self.dlg)
    ...
    def openProjectDlg(self):
        ...
        if self.okToContinue():
            self.projFileName = QtGui.QFileDialog.getOpenFileName(
                self,
                "Select a file",
                self.workingDir,
                ("QuShape project file (*.qushapey *.qushape *.pyshape *.txt *.fsa )"),
            )
            if not self.projFileName.isEmpty():
                ...
                try:
                    ...
                    msg = (
                        "Selected files is not appropriate to open in QuShape.\n"
                        + str(e)
                    )
    ...
    def openProject(self, projFile=None):
        ...
        self.dProject, self.dVar, self.intervalData, self.dProjRef = openProjFile(
            self.projFileName
        )
    ...
    def saveProject(self):
        ...
        with open(self.projFileName, "w") as fd:
            yaml.dump(
                self.dBase,
                fd,
            )
    ...
    def saveProjectAs(self):
        fName = QtGui.QFileDialog.getSaveFileName(
            self, "Save As", self.dProject["dir"], ""
        )
        if not fName.isEmpty():
            self.projFileName = str(fName) + ".qushapey"
    ...
    def saveCurLane(self):
        fileName = QtGui.QFileDialog.getSaveFileName(
            self, "Save As", self.dProject["dir"], "Text Files (*.txt)"
        )
        if not fileName.isEmpty():
            saveCurLaneAsTxt(
                fileName, self.dDrawData["dData"], self.dProject["chKeyRS"]
            )
    ...
    def okToContinue(self):
        if self.dirty:
            msg = (
                ...
                + str(self.dProject["name"])
                + " project?"
            )
            reply = QtGui.QMessageBox.question(
                self,
                "PyShape - Unsaved Changes",
                msg,
    ...
    def drawDlgApply(self):
        ...
        if self.dlg.name == "Reactivity":
            ...
            self.dlg.name == "Sequence Alignment by Reference"
            or self.dlg.name == "Automated by Reference"
        ):
            self.dVar["flag"]["isSeqAlign"] = True
            self.dVar["flag"]["isDrawLine"] = True
            self.drawFigure()
    ...
    def clickedDone(self, next=True):
        if self.dlg.name == "Open ABIF File":
            ...
            self.dlg.name == "Reactivity by Reference"
            or self.dlg.name == "Reactivity"
            or self.dlg.name == "Automated by Reference"
        ):
            self.dProjRef = deepcopy(self.dlg.dProjRef)
    ...
    def setToolDlg(self):
        self.connect(
            self.dlg.buttonBox.applyButton,
            QtCore.SIGNAL("clicked()"),
            self.clickedApply,
        )
        self.connect(
            self.dlg.buttonBox.doneButton, QtCore.SIGNAL("clicked()"), self.clickedDone
        )
        self.connect(
            self.dlg.buttonBox.skipButton, QtCore.SIGNAL("clicked()"), self.clickedSkip
        )
        self.dockTool.setWidget(self.dlg)
    ...
    def setAction(self):
        ...
        if self.curScript == "Region of Interest":
            ...
            self.spanROI = {}
            self.rectRX = self.chAxes["RX"].axvspan(
                self.dlg.roi["RX"][0],
                self.dlg.roi["RX"][1],
                visible=True,
                facecolor="0.8",
            )
            self.rectRXS1 = self.chAxes["RXS1"].axvspan(
                self.dlg.roi["RX"][0],
                self.dlg.roi["RX"][1],
                visible=True,
                facecolor="0.8",
            )
            self.rectBG = self.chAxes["BG"].axvspan(
                self.dlg.roi["BG"][0],
                self.dlg.roi["BG"][1],
                visible=True,
                facecolor="0.8",
            )
            self.rectBGS1 = self.chAxes["BGS1"].axvspan(
                self.dlg.roi["BG"][0],
                self.dlg.roi["BG"][1],
                visible=True,
                facecolor="0.8",
            ...
            self.connect(
                self.dlg.button0, QtCore.SIGNAL("clicked()"), self.drawSignalAlignModify
            )
            ...
            self.connect(
                self.dlg.pushButton0,
                QtCore.SIGNAL("clicked()"),
                self.drawReactivityPressed,
            )
            self.connect(
                self.dlg.pushButton1,
                QtCore.SIGNAL("clicked()"),
                self.drawReactivityPressed,
            )
            self.connect(
                self.dlg.pushButton2,
                QtCore.SIGNAL("clicked()"),
                self.drawReactivityPressed,
            )
            self.dlg.initialize()
            ...
            self.connect(
                self.dlg.button1, QtCore.SIGNAL("clicked()"), self.drawPeakLinkRefModify
            )
    ...
    def viewReport(self):
        ...
        self.connect(
            ...
            self.showSelectedNucInReport,
        )

    def showSelectedNucInReport(self):
        self.canvas.restore_region(self.background)
        nucPos = self.dlg.dReport["posSeq"][
            self.dlg.table.currentRow()
        ]  # self.dlg.table.item(curRow,1)
        for ax, rect in zip(self.fig.get_axes(), self.spanRect):
            rect.set_width(11)
            rect.set_x(nucPos - 5)
            ax.draw_artist(rect)

        newScrollValue = (nucPos * self.mainTopWidget.spinBoxWidth.value() / 100) - (
            self.scrollArea.horizontalScrollBar().pageStep() / 2
        )
    ...
    def drawROISpan(self):
        ...
        self.rectRX = self.chAxes["RX"].axvspan(
            self.dlg.roi["RX"][0], self.dlg.roi["RX"][1], visible=True, facecolor="0.8"
        )
        self.rectRXS1 = self.chAxes["RXS1"].axvspan(
            self.dlg.roi["RX"][0], self.dlg.roi["RX"][1], visible=True, facecolor="0.8"
        )
        self.rectBG = self.chAxes["BG"].axvspan(
            self.dlg.roi["BG"][0], self.dlg.roi["BG"][1], visible=True, facecolor="0.8"
        )
        self.rectBGS1 = self.chAxes["BGS1"].axvspan(
            self.dlg.roi["BG"][0], self.dlg.roi["BG"][1], visible=True, facecolor="0.8"
        )
    ...
    def drawSignalAlignModify(self):
        self.conFromRtoS, self.axesR, self.axesS = drawPeakLinkModifyFig(
            self.fig, self.dlg.dataR, self.dlg.dataS, self.dlg.linkXR, self.dlg.linkXS
        )
    ...
    def drawPeakLinkRefModify(self):
        ...
        self.dVar["flag"]["isSeqAlign"] = True
        self.drawFigure()
        self.axesSeq = self.fig.add_axes(
            [
                self.dFigMargin["L"],
                0.035,
                self.dFigMargin["R"] - self.dFigMargin["L"],
                0.035,
            ]
        )  # ,axisbg='#FFFFCC')
    ...
    def drawGaussFit(self, dProject):
        ...
        for key in keys:
            ...
            for i in range(len(dProject[key0]["pos"])):
                x = np.arange(
                    dProject[key0]["pos"][i] - 2 * dProject[key0]["wid"][i],
                    dProject[key0]["pos"][i] + 2 * dProject[key0]["wid"][i],
                )
                A1 = fitFuncG(
                    x,
    ...
    def drawMatchLines(self, dProject):
        self.conFromBGToRX = drawMatchLines0(
            dProject["dPeakBG"]["pos"],
            dProject["dPeakBG"]["amp"],
            dProject["dPeakRX"]["pos"],
            dProject["dPeakRX"]["amp"],
            self.chAxes["BG"],
            self.chAxes["RX"],
        )
        self.conFromSeqToBG = drawMatchLines0(
            dProject["seqX"],
            dProject["dData"]["BGS1"][dProject["seqX"]],
            dProject["dPeakBG"]["pos"],
            dProject["dPeakBG"]["amp"],
            self.chAxes["BGS1"],
            self.chAxes["BG"],
    ...
    def drawReactivityFigures(self):
        if self.dlg.name == "Reactivity":
            if self.drawReactivityType == 2:
                ...
                self.drawReactivityFig(
                    self.dlg.dProjOut,
                    self.dlg.radio5to3.isChecked(),
                    self.drawReactivityType,
                )
        if (
            self.dlg.name == "Reactivity by Reference"
            or self.dlg.name == "Automated by Reference"
            ...
            if self.drawReactivityType == 2:
                ...
                self.drawReactivityRef(
                    self.dlg.dProjOut, self.dlg.dProjRef, self.drawReactivityType
                )
    ...
    def drawPeaks(self, dProject):
        ...
        for key in dProject["chKeyRS"]:
            key1 = str("dPeak" + key)
            (self.dLinePeak[key],) = self.chAxes[key].plot(
                dProject[key1]["pos"], dProject[key1]["amp"]
            )
    ...
    def drawRefData(self, refData):
        try:
            ...
            for key in refData["dData"].keys():
                factor = np.average(self.dDrawData["dData"][key]) / np.average(
                    refData["dData"][key]
                )
                (line,) = self.chAxes[key].plot(
                    refData["dData"][key] * factor
                )  # ,'0.5')
    ...
    def drawFigure(self):
        ...
        if self.dVar["flag"]["isDrawStad"]:
            self.drawSatd()
        try:
            self.dAxesYLim = findAxesYLim(
                self.dDrawData["dData"], self.dVar["drawType"]
            )
        except:
            pass
    ...
    def setAxesYLim(self):
        for key in self.dDrawData["dData"].keys():
            ...
            y1 = self.dAxesYLim[key][1] / (
                self.mainTopWidget.spinBoxZoom.value() * 0.01
            )
    ...
    def drawSatd(self):
        for i in range(len(self.dDrawData["Satd"]["RX"])):
            self.chAxes["RX"].axvline(
                self.dDrawData["Satd"]["RX"][i], alpha=0.3, color="red"
            )
            self.chAxes["RXS1"].axvline(
                self.dDrawData["Satd"]["RX"][i], alpha=0.3, color="red"
            )
        for i in range(len(self.dDrawData["Satd"]["BG"])):
            self.chAxes["BG"].axvline(
                self.dDrawData["Satd"]["BG"][i], alpha=0.3, color="red"
            )
            self.chAxes["BGS1"].axvline(
                self.dDrawData["Satd"]["BG"][i], alpha=0.3, color="red"
            )
        self.canvas.draw()
    ...
    def drawSelectedArea(self):
        ...
        self.canvas.resize(
            2 * self.spanRect[0].get_width(), self.canvas.size().height()
        )
        self.canvas.draw()
    ...
    def applySplitCombo(self):
        if len(self.fig.get_axes()) < 1:
            return True
        if (
            self.dVar["flag"]["isPeakMatchModify"]
            or self.dVar["flag"]["isPeakLinkRefModify"]
            or self.dVar["flag"]["isDrawRef"]
        ):
            return True
    ...
    def setLineColor(self):
        for key in self.dVar["lineColor"].keys():
            self.mainTopWidget.labelCh[key].setStyleSheet(
                "QWidget { background: %s }" % self.dVar["lineColor"][key]
            )
    ...
    def editLineProps(self):
        ...
        self.connect(
            self.dlgChannel.buttonBox.button(QtGui.QDialogButtonBox.Apply),
            QtCore.SIGNAL("clicked()"),
            self.applyEditLineProps,
        )
