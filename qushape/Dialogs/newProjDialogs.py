...
class DlgNewProject0(QtGui.QDialog):
    def __init__(self, dProject, parent=None):
        ...
        self.title = QtGui.QLabel(
            self.tr("<center><b>CREATE NEW PROJECT - STEP 1 OF 3</b></center>")
        )
        ...
        self.connect(
            self.buttonBox.nextButton, QtCore.SIGNAL("clicked()"), self.clickNext0
        )
    ...
    def clickNext0(self):
        if self.lineEdit0.text() == "":
            ...
            self.dProject["fName"] = (
                self.dProject["dir"] + "/" + self.dProject["name"] + ".qushape"
            )
...
class DlgNewProject1(QtGui.QDialog):
    def __init__(self, dProject, parent=None):
        ...
        self.name = "New Project"
        self.title = QtGui.QLabel(
            self.tr("<center><b>CREATE NEW PROJECT - STEP 2 OF 3</b></center>")
        )

        self.dProject = dProject
        self.isApplied = False

        self.fileRead0 = DlgSelectFile(
            "(+) Reaction",
            "ABIF or Text File (*.ab1 *.fsa *.txt)",
            self.dProject["dir"],
        )
        self.fileRead1 = DlgSelectFile(
            "(-) Reaction",
            "ABIF or Text File (*.ab1 *.fsa *.txt)",
            self.dProject["dir"],
        )
        self.fileRead2 = DlgSelectFile(
            "Sequence", "Base Files (*.txt *.fasta *.gbk *.seq )", self.dProject["dir"]
        )
        self.fileRead3 = DlgSelectFile(
            "Ref. Proj.",
            "Reference Project (*.pyshape *.qushape *.qushapey)",
            self.dProject["dir"],
        )
        ...
        self.connect(
            self.buttonBox.backButton, QtCore.SIGNAL("clicked()"), self.clickBack1
        )
        self.connect(
            self.buttonBox.nextButton, QtCore.SIGNAL("clicked()"), self.clickNext1
        )
    ...
    def clickNext1(self):
        if self.fileRead0.lineEdit0.text() == "":
            ...
            self.fileRead2.lineEdit0.text() == ""
            and self.fileRead3.lineEdit0.text() == ""
        ):
            QtGui.QMessageBox.warning(
                self, "Warning", "Select Sequence or Reference File"
            )
        else:
            self.readFileNames()
...
class DlgNewProject2(QtGui.QDialog):
    def __init__(self, dProject, parent=None):
        ...
        self.name = "New Project"
        self.title = QtGui.QLabel(
            self.tr("<center><b>CREATE NEW PROJECT - STEP 3 OF 3</b></center>")
        )
        self.dProject = dProject
        self.isApplied = False
        ...
        self.connect(
            self.buttonBox.backButton, QtCore.SIGNAL("clicked()"), self.clickBack2
        )
        self.connect(
            self.buttonBox.nextButton, QtCore.SIGNAL("clicked()"), self.clickNext2
        )
    ...
    def clickNext2(self):
        dataRX, self.dProject["Satd"]["RX"], dyesRX = readShapeData(
            self.dProject["fNameRX"]
        )
        dataBG, self.dProject["Satd"]["BG"], dyesBG = readShapeData(
            self.dProject["fNameBG"]
        )
        ...
        if self.dProject["fNameRef"] == "":
            ...
            extFile = QtCore.QFileInfo(self.dProject["fNameRef"]).suffix()
            if extFile == "qushapey":
                with open(self.dProject["fNameRef"], "r") as fd:
        ...
        for key in self.comboBox0.keys():
            ...
            if key == "RX" or key == "RXS1" or key == "RXS2":
                ...
                self.dProject["dyeN"][key] = dyesRX[
                    ind
                ]  # str(readerRX.getData('DyeN',ind+1)).replace(' ','')
            else:
                self.dProject["dData"][key] = dataBG[:, ind]
                self.dProject["dyeN"][key] = dyesBG[
                    ind
                ]  # str(readerBG.getData('DyeN',ind+1)).replace(' ','')
...
class DlgProjInfo(QtGui.QWidget):
    def __init__(self, dProject, parent=None):
        ...
        self.labelTitle = QtGui.QLabel(
            self.tr("<center><b>PROJECT INFORMATION</b></center>")
        )
        self.name = "ProjInfo"
        self.toolID = 1
        ...
        self.projInfo += (
            "<p><b> RNA File: </b>" + str(self.dProject["fNameSeq"]) + "</p>"
        )
        self.projInfo += (
            "<p><b> Reference File: </b>" + str(self.dProject["fNameRef"]) + "</p>"
        )
        ...
        self.projInfo += "<p><b> Channel Index: RX: </b>" + str(
            self.dProject["chIndex"]["RX"] + 1
        )
