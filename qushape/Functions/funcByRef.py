...
def seqAlignRef(
    dProjRef, dProjOut, keyR, keyS, method="peakSim", isNormalize=True, derivative=True
):
...
def applyAllSeq(dProject, dProjRef):
    ...
    for key in dProjOut["chKeyRS"]:
        dProjOut["dData"][key] = splineSampleData(
            dProjOut["dData"][key], dProjRef["dData"][key], linkXR, linkXS, False
        )
    ...
    dProjOut["dPeakBG"], controlRX = peakLinking(
        dProjRef["dPeakBG"]["pos"], dProjOut["dPeakBG"], dProjOut["dData"]["BG"]
    )
    dProjOut["dPeakRX"], controlBG = peakLinking(
        dProjRef["dPeakRX"]["pos"], dProjOut["dPeakRX"], dProjOut["dData"]["RX"]
    )
    ...
    if len(dProjOut["dPeakRX"]["amp"]) > 160:
        scaleFactor0 = scaleShapeDataWindow(
            dProjRef["dPeakRX"]["area"], dProjOut["dPeakRX"]["area"]
        )
        scaleFactorData0 = fitLinear(
            dProjOut["dPeakRX"]["pos"], scaleFactor0, NData=len(dProjOut["dData"]["RX"])
        )
    else:
        scaleFactor0 = scaleShapeData(
            dProjRef["dPeakRX"]["area"], dProjOut["dPeakRX"]["area"], rate=1
        )
    ...
    if len(dProjOut["dPeakBG"]["amp"]) > 160:
        scaleFactor0 = scaleShapeDataWindow(
            dProjRef["dPeakBG"]["area"], dProjOut["dPeakBG"]["area"]
        )
        scaleFactorData0 = fitLinear(
            dProjOut["dPeakBG"]["pos"], scaleFactor0, NData=len(dProjOut["dData"]["BG"])
        )
    else:
        scaleFactor0 = scaleShapeData(
            dProjRef["dPeakBG"]["area"], dProjOut["dPeakBG"]["area"], rate=1
        )
        scaleFactorData0 = scaleFactor0
    ...
    if len(dProjOut["normDiff"]):
        ...
        scaleFactor2 = scaleShapeData(
            dProjRef["normDiff"], dProjOut["areaDiff"], rate=1
        )
...
def applyAllToolsAuto1(dProject, dProjRef):
    ...
    for key in dProject["chKeyRX"]:
        dProjOut["dData"][key] = correctSatd(
            dProjOut["dData"][key], dProjOut["Satd"]["RX"]
        )
    for key in dProject["chKeyBG"]:
        dProjOut["dData"][key] = correctSatd(
            dProjOut["dData"][key], dProjOut["Satd"]["BG"]
        )
    ...
    dProjOut["dData"]["RXS1"] = fMobilityShift(
        dProjOut["dData"]["RX"], dProjOut["dData"]["RXS1"], dyeNR, dyeNS
    )
    if "RXS2" in dProject["dData"].keys():
        dyeWS = dDyesWL[dProject["dyeN"]["RXS2"]]
        dProjOut["dData"]["RXS2"] = fMobilityShift(
            dProjOut["dData"]["RX"], dProjOut["dData"]["RXS2"], dyeNR, dyeNS
        )

    dyeNR = dProject["dyeN"]["BG"]
    dyeNS = dProject["dyeN"]["BGS1"]
    dProjOut["dData"]["BGS1"] = fMobilityShift(
        dProjOut["dData"]["BG"], dProjOut["dData"]["BGS1"], dyeNR, dyeNS
    )
    if "BGS2" in dProject["dData"].keys():
        dyeWS = dDyesWL[dProject["dyeN"]["BGS2"]]
        dProjOut["dData"]["BGS2"] = fMobilityShift(
            dProjOut["dData"]["BG"], dProjOut["dData"]["BGS2"], dyeNR, dyeNS
        )
...
if __name__ == "__main__":
    global is_legacy_files_available
    import sys
    import numpy as np
    ...
    extFile = QtCore.QFileInfo(fNameR).suffix()
    if extFile == "qushapey":
        with open(fNameR, "r") as fd:
        ...
        if is_legacy_files_available:
            ...
            dialog.setText(
                "Legacy format .qushape is not available because shelve+bsddb3 are not correctly installed, please use .qushapey format"
            )
            dialog.exec_()
    ...
    extFile = QtCore.QFileInfo(fNameS).suffix()
    if extFile == "qushapey":
        with open(fNameS, "r") as fd:
            dBase = yaml.load(fd, Loader=yaml.Loader)
    else:
        if is_legacy_files_available:
    ...
    key = "BG"
    dataSOut, dataR = applyAllToolsAuto0(
        dProjRaw["dData"][key], dProjR["dData"][key], dProjRaw["Satd"][key]
    )
